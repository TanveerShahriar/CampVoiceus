...
const HomeThread: React.FC<GroupThreadProps> = ({ thread }) => {
    const [stateThread, setStateThread] = useState<GroupThread>(thread);
    ...
    const handleUpvote = async (threadId : string) => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}/groups/upvote`, upvoteData,
                {
                    headers: {
    ...
    const handleDownvote = async (threadId : string) => {
        ...
        try {
            const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}/groups/downvote`, downvoteData,
                {
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${token}`,
    ...
    const handleFileDownload = async (threadId : string) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(
                `${import.meta.env.VITE_SERVER_URL}/groups/filedownload`,
                { threadId },
                {
                    headers: {
